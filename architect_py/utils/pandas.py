...
def candles_to_dataframe(candles: List["Candle"]) -> pd.DataFrame:
    ...
    if df.empty:
        columns = pd.Index(["timestamp", *CANDLES_FIELD_MAP.values()])
...
def tickers_to_dataframe(tickers: List["Ticker"]) -> pd.DataFrame:
    ...
    if df.empty:
        columns = pd.Index(["timestamp", *TICKERS_FIELD_MAP.values()])
        df = pd.DataFrame(columns=columns)
        df.set_index("symbol", inplace=True)
        return df

    df.rename(columns=TICKERS_FIELD_MAP, inplace=True)
    df["timestamp"] = pd.to_datetime(
        df["ts"] * 1_000_000_000 + df["tn"],
        unit="ns",
        utc=True,
